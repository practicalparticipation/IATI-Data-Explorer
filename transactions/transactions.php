<?php
  ...
  //The main script
   ...
   $prefix = "http://projects.dfid.gov.uk/iati/project/";
   if($_GET['q']) { 
	$query = $_GET['q'];
   } else {
	$complete = 1;
	exError("No project ID was given");
	exit();
   }
  ...
  if(file_exists($filename)) {
   ...
   if($xmlDoc->getElementsByTagName('iati-activity')->item(0)) {
	   $xslDoc = new DOMDocument();
   	   $xslDoc->load("iati-project-transactions.xsl");

	   $proc = new XSLTProcessor();
	   $proc->importStylesheet($xslDoc);

	   $outdir = str_replace("transactions.php","",$_SERVER["SCRIPT_FILENAME"]);
